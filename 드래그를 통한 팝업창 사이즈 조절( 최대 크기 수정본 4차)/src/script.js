...
function enableResize(popupElement) {
    ...
    // 최대 및 최소 크기 설정
    const MIN_SIZE = 150; // 최소 크기 (가로와 세로 동일)
    ...
    document.addEventListener('mousemove', (e) => {
        if (isResizing) {
            ...
            // 마우스 이동에 민감도를 더해줌
            const speedFactor = 4;  // 이 값을 조정하여 속도 민감도를 조절
            const delta = Math.max(deltaX, deltaY) * speedFactor;

            let newSize = startWidth + delta; // 새 크기 계산 (가로와 세로 동일하게)
            newSize = Math.min(Math.max(newSize, MIN_SIZE+200), MAX_SIZE-210); // 크기 제한 적용

            // 팝업 크기 조정
            popupElement.style.width = `${newSize}px`;
            popupElement.style.height = `${newSize}px`;
...
function adjustCloseButton(popupElement, newWidth, newHeight) {
    const closeButton = popupElement.querySelector('close-button'); // 커스텀 태그 선택

    // 기본 크기 설정
    const baseWidth = 600; // 초기 팝업 너비
    const baseHeight = 400; // 초기 팝업 높이
    const basePadding = 10; // 초기 패딩 값
    const baseFontSize = 16; // 초기 폰트 크기
    const baseTop = 35; // 초기 상단 위치
    const baseRight = 40; // 초기 오른쪽 위치
    ...
    if (closeButton) {
        // 패딩 및 폰트 크기 조정
        closeButton.style.padding = `${basePadding * heightScale}px ${basePadding * widthScale * 2}px`;
        closeButton.style.fontSize = `${baseFontSize * widthScale}px`;

        // 위치 조정
        closeButton.style.top = `${baseTop * heightScale}px`;
        closeButton.style.right = `${baseRight * widthScale}px`;
    }
}
...
function adjustPopupContent(popupElement, newWidth, newHeight) {
    ...
    const title = popupElement.querySelector('h2'); // '캐릭터 정보' 텍스트
    const closeButton = popupElement.querySelector('.close-button'); // 닫기 버튼
    ...
    const baseTitleFontSize = 20; // '캐릭터 정보' 텍스트 기본 폰트 크기
    const baseButtonSize = 30; // 닫기 버튼의 기본 크기

    // 비율에 따라 크기 조정
    const scaleFactorWidth = newWidth / baseWidth; // 너비 비율
    const scaleFactorHeight = newHeight / baseHeight; // 높이 비율
    ...
    // '캐릭터 정보' 텍스트 크기 조정
    if (title) {
        const newTitleFontSize = baseTitleFontSize * scaleFactorWidth; // 폰트 크기 조정
        title.style.fontSize = `${newTitleFontSize}px`;
    }

    // 닫기 버튼 크기와 위치 조정
    if (closeButton) {
        const newButtonSize = baseButtonSize * scaleFactorWidth; // 너비 비율로 크기 조정
        closeButton.style.width = `${newButtonSize}px`;
        closeButton.style.height = `${newButtonSize}px`;

        // 닫기 버튼 위치 (우측 상단)
        closeButton.style.top = `${10 * scaleFactorHeight}px`; // 상단 여백
        closeButton.style.right = `${10 * scaleFactorWidth}px`; // 우측 여백
        closeButton.style.fontSize = `${newButtonSize * 0.5}px`; // 'X' 텍스트 크기 조정
    }
    adjustCloseButton(popupElement, newWidth, newHeight);
}
