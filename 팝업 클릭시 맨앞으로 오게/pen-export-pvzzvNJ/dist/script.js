...
function searchCharacter() {
    ...
    const headers = {
        "x-nxopen-api-key": "test_fe10bb3cbd4eb67284a316aaa4a855d283d3e35d29d93d32c8f3119a9426a202efe8d04e6d233bd35cf2fabdeb93fb0d" // 발급받은 nxopen_api_key 입력
...
function adjustPopupPosition(popupElement) {
    ...
    // 화면의 아래쪽에 팝업이 겹치지 않도록 위치 조정
    let topPosition = window.innerHeight - popupElement.offsetHeight - footerRect.height - 400; // footer 바로 위로 설정
    topPosition = Math.max(topPosition, 20); // 화면 상단과 너무 가까운 경우 상단 여백을 20px로 설정

    popupElement.style.position = 'fixed';
    popupElement.style.top = `${topPosition}px`;
    popupElement.style.left = '20px'; // 좌측 20px 여백
}
