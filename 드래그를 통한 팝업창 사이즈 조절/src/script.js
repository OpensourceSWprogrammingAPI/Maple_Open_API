...
function enableResize(popupElement) {
    ...
    document.addEventListener('mousemove', (e) => {
        if (isResizing) {
            ...
            // 크기 제한 적용
            newWidth = Math.min(Math.max(newWidth - 130, MIN_WIDTH + 150), MAX_WIDTH); // 최소와 최대 너비 적용
            newHeight = Math.min(Math.max(newHeight - 130 , MIN_HEIGHT + 150), MAX_HEIGHT); // 최소와 최대 높이 적용

            popupElement.style.width = `${newWidth}px`;
            popupElement.style.height = `${newHeight}px`;

            // 팝업 콘텐츠 크기 조정
            adjustPopupContent(popupElement, newWidth, newHeight);
        }
    });
...
function adjustPopupContent(popupElement, newWidth, newHeight) {
    const image = popupElement.querySelector('#character_image');
    const details = popupElement.querySelector('.character-detail');
    const imageContainer = popupElement.querySelector('.image-container');

    // 팝업 내 이미지 크기와 텍스트 폰트 크기 업데이트
    if (image && imageContainer) {
        imageContainer.style.width = `${newWidth * 0.4}px`;
        imageContainer.style.height = `${newHeight * 0.4}px`;
        image.style.width = '100%';
        image.style.height = '100%';
    }

    if (details) {
        const fontSize = Math.max(10, newWidth / 40); // 폰트 크기 최소값 10px
        details.style.fontSize = `${fontSize}px`;
        details.style.lineHeight = `${fontSize * 1.2}px`; // 줄 간격 조정
    }

    // 팝업이 화면 경계 밖으로 나가지 않도록 위치 재조정
    const popupRect = popupElement.getBoundingClientRect();
    const maxWidth = window.innerWidth;
    const maxHeight = window.innerHeight;

    if (popupRect.right > maxWidth) {
        popupElement.style.left = `${maxWidth - popupRect.width}px`;
    }
    if (popupRect.bottom > maxHeight) {
        popupElement.style.top = `${maxHeight - popupRect.height}px`;
    }
}
